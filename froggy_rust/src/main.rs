...
impl State {
    ...
    fn play(&mut self, ctx: &mut BTerm) {
        ...
        // player logic
        self.frog.render(ctx);
        ctx.print(0,0, "press SPACE to jump");
        ctx.print(SCREEN_WIDTH-3,0, &format!("{}", self.score));

        // obstacle logic
        let mut rng = rand::thread_rng();
        if self.cacti.is_empty() || self.frog.x + SCREEN_WIDTH - self.cacti[self.cacti.len() - 1].x > rng.gen_range(5..self.frog.x + 50){
            self.score += 1;
            self.cacti.push(Cactus::new(
                self.frog.x + SCREEN_WIDTH,
                rand::thread_rng().gen_range(0..3)
            ));
        }
        for cactus in self.cacti.iter_mut() {
            cactus.render(ctx, self.frog.x);
            if cactus.hit_obstacle(&mut self.frog) {
